...
# --- Frontend deploy permissions for GitHub Actions ---
...
resource "aws_iam_policy" "frontend_s3" {
  name        = "${var.project}-frontend-s3"
  description = "Allow GitHub Actions to sync frontend to S3 and invalidate CloudFront"
  policy      = data.aws_iam_policy_document.frontend_s3.json
}

resource "aws_iam_role_policy_attachment" "gha_frontend_s3" {
  role       = data.aws_iam_role.gha.name
  policy_arn = aws_iam_policy.frontend_s3.arn
}

# Optional CloudFront invalidation permission
...
resource "aws_iam_policy" "frontend_cf" {
  count       = var.cloudfront_distribution_id != "" ? 1 : 0
  name        = "${var.project}-frontend-cf"
  description = "Allow GitHub Actions to invalidate CloudFront"
  policy      = data.aws_iam_policy_document.frontend_cf[0].json
}

resource "aws_iam_role_policy_attachment" "gha_frontend_cf" {
  count      = var.cloudfront_distribution_id != "" ? 1 : 0
  role       = data.aws_iam_role.gha.name
  policy_arn = aws_iam_policy.frontend_cf[0].arn
}
